...
В качестве исходных данных используется одно полноцветное изображение страницы, загруженное с сайта `https://www.slavcorpora.ru` через API выборки образцов. Для ЛР4 выбрана страница с индексом, отличным от использованных во 2 и 3 лабораторных работах.
...
Максимальное значение модуля градиента для выбранного изображения оказалось \(\max G \approx 2652\). Порог бинаризации выбран по формуле:
...
4. По сравнению с морфологическим выделением контуров (ЛР3) градиентный подход с оператором Кайяли даёт более тонкие и локализованные края, лучше подчёркивая реальные переходы яркости, но при этом остаётся более чувствительным к шумам и параметру порога.

# Лабораторная работа №4

## Выделение контуров. Вариант 14 — оператор Кайяли 3×3, \(G = |G_x| + |G_y|\)
