...
Для каждой буквы сформирован вектор признаков (отношение сторон, компактность и логарифмическая шкала по модулям инвариантов $\varphi_k$). Попарные расстояния:
...
На рисунке строки и столбцы упорядочены как алфавит слева направо; после нормировки на $M$ отображается величина

$$
1 - \dfrac{d_{ij}}{M}
$$

(чем **темнее** пиксель, тем **меньше** расстояние между символами).
