...
Испанский алфавит **заглавными** буквами: `ABCDEFGHIJKLMNÑOPQRSTUVWXYZ` (27 символов, включая **Ñ**). Эталонное изображение синтезируется шрифтом **Times New Roman**, кегль **52 pt** (параметр `--font-size` в `lab5/codeFor5.py`, путь к `.ttf` — `--font`). Выполняются **бинаризация Оцу**, выделение ROI символов, расчёт **геометрических признаков**, **инвариантов Ху**, **горизонтальной и вертикальной проекций**, построение **матрицы евклидовых расстояний** между векторами признаков. Каждая буква сохраняется отдельным файлом `lab5/src/char_<буква>.png` (для **Ñ** — `char_Ntilde.png`).
...
Для бинарного изображения объекта задаётся маска \(I(x, y) \in \{0, 1\}\) (в реализации единице соответствуют пиксели чернил после пороговой обработки).
...
**Инварианты Ху** \(\varphi_1, \ldots, \varphi_7\) строятся из центральных моментов; в задачах распознавания они дают описание формы, устойчивое к сдвигу, изменению масштаба и (в непрерывной модели) к повороту.
...
Символы сформированы средствами `PIL.ImageDraw` и `ImageFont.truetype`, фон светлый, чернила тёмные.

![Полутоновая строка алфавита](src/alphabet_grayscale.png)

### 2. Бинаризация (Оцу)

Для полученного полутона автоматически найден порог Оцу \(T = 134\). Пиксель отнесён к объекту (чернилам), если яркость полутона удовлетворяет условию \(I_{\mathrm{gray}}(x, y) \le T\).
...
### 4. Монтаж всех символов в один ряд

![Монтаж символов](src/alphabet_montage.png)

### 5. Проекции (примеры для A, Ñ, I)
...
Обозначения: \(S\) — площадь; \(W \times H\) — габариты ограничивающего прямоугольника после обрезки; \(a = W/H\); \(C = P^2/S\) — компактность; \(\varphi_1, \varphi_2\) — первые два инварианта Ху.
...
Полный набор признаков, включая \(\varphi_3, \ldots, \varphi_7\) и координаты центра масс, — в `lab5/src/features.json`; текстовая сводка — в `lab5/src/features_table.txt`.

### 7. Матрица расстояний между символами

Для каждой буквы сформирован вектор признаков (отношение сторон, компактность и логарифмическая шкала по модулям \(\varphi_k\)). Попарные расстояния:

$$
d_{ij} = \left\| \mathbf{v}_i - \mathbf{v}_j \right\|_2.
$$

На рисунке строки и столбцы упорядочены как алфавит слева направо; после нормировки на \(\max_{i,j} d_{ij}\) отображается величина \(1 - d_{ij} / \max d\) (чем **темнее** пиксель, тем **меньше** расстояние между символами).

![Матрица расстояний](src/distance_matrix.png)
...
3. Вычислены **площадь**, **габариты**, **\(a = W/H\)**, **компактность**, **инварианты Ху** и построены **проекции** для примеров **A**, **Ñ**, **I**.
