...
- слева — `до` логарифмического контрастирования, справа — `после`;
- после преобразования заметно усиливаются области с большими длинами серий (рост вклада длинных пробегов), что согласуется с увеличением `LRE`.
...
Подпись к паре `GLRLM`:
- слева — `до`, справа — `после` логарифмического преобразования;
- распределение после обработки становится менее концентрированным у самых коротких серий и более выраженным в средних/длинных длинах, что отражает снижение `SRE` и рост `LRE`.
...
Подпись к паре `GLRLM`:
- слева — матрица исходного изображения, справа — после логарифмического контрастирования;
- после обработки возрастает видимость структур в области более длинных серий и немного уменьшается доминирование коротких серий (что подтверждается изменениями `SRE`/`LRE`).
